                                ...
                                <div class="card eg_card p-2">
                                    ...
                                    <div class="card-body">
                                        <div class="motion_text">
                                            ...
                                            <span class="small text-muted">{{ $viewData->groupAcceptedMemberCount($group) }} {{ get_phrase('Member') }}{{ $viewData->groupAcceptedMemberCount($group)>1?"s":"" }}</span>
                                        </div>
                                        ...
                                        {{-- <span class="small text-muted">{{ $viewData->groupAcceptedMemberCount($group) }} Member @if($viewData->groupAcceptedMemberCount($group)>1) s @endif</span> --}}
                                         <div class="join_groups">
                                                @if ($viewData->userJoinedGroup($group, auth()->user()))
                                                    ...
                                                    @else
                                                        <a href="javascript:void(0)" onclick="ajaxAction('{{ route('group.rjoin',$group->id) }}')" class="j_btn btn btn-secondary">
                                                    ...
                                                    @endif
                                                @else
                                                    <a href="javascript:void(0)" onclick="ajaxAction('{{ route('group.join',$group->id) }}')" class="btn btn-primary">
                                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
