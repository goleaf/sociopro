@if ($user_data->id !== auth()->user()->id)
    <div class="widget page-widget ac_control_we">
        <div class="user_ac">
            @if ($viewData->friendshipStatus($user_data, auth()->user())['exists'])
                @if ($viewData->friendshipStatus($user_data, auth()->user())['accepted'])
                    <a href="#" class="btn common_btn ac_btn" id="btnGroupDrop1" data-bs-toggle="dropdown"
                        aria-expanded="false"><i class="fa-solid fa-user-group"></i> {{ get_phrase('Friend') }}</a>
                    <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                        <li><a href="javascript:void(0)"
                                ...
                                {{ get_phrase('Unfriend') }}</a></li>
                        <li>
                            @if ($viewData->friendshipStatus($user_data, auth()->user())['is_following'])
                ...
                @else
                    <form class="ajaxForm" action="{{ route('profile.accept_friend_request') }}" method="post">
                        <a href="javascript:void(0)"
                            ...
                            title="{{ get_phrase('Cancle Friend Request') }}"><i class="fa-solid fa-xmark"></i>
                            {{ get_phrase('Cancel') }}</a>
                        @CSRF
                        <input type="hidden" name="user_id" value="{{ $user_data->id }}">
                        @if ($viewData->friendshipStatus($user_data, auth()->user())['requester_id'] === auth()->user()->id)
                            <button type="button" class="btn common_btn_2 px-4 no-processing no-uploading"
                                data-bs-toggle="tooltip" data-bs-placement="top"
                                title="{{ get_phrase('Friend request') }}">{{ get_phrase('Requested') }}</button>
                        @else
                            <button type="submit" id="friendRequestConfirmBtn{{ $user_data->id }}"
                                ...
                                data-bs-placement="top" title="{{ get_phrase('Accept Friend Request') }}">
                                {{ get_phrase('Accept') }}
                            </button>
                        @endif
                        <button type="button" id="friendRequestAcceptedBtn{{ $user_data->id }}"
                            class="btn common_btn d-hidden ac_btn px-4" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="{{ get_phrase('Accepted') }}">{{ get_phrase('Accepted') }}</button>
                    </form>
                @endif
            @else
                <a href="javascript:void(0)" onclick="ajaxAction('{{ route('user.friend', $user_data->id) }}')"
                    class="btn common_btn ac_btn"><i class="fa-solid fa-plus"></i> {{ get_phrase('Add Friend') }}</a>
            @endif
        </div>
    </div>
@endif
...
<aside class="sidebar">
    ...
    <div class="widget" id="my-profile-info">
        ...
        <ul>
            <li><i class="fa fa-briefcase"></i> <span><strong>{{ $user_data->job }}</strong></span></li>
    ...
    <div class="widget">
        ...
        <div id="sidebarPhotoAndVideos" class="row row-cols-3 g-1 mt-3">
            @foreach ($viewData->profileMediaFiles($user_data->id) as $media_file)
                ...
                @else
                    <div class="single-item-countable col">
                        <a href="{{ route('single.post', $media_file->post_id) }}">
                            <img class="img-thumbnail w-100 user_info_custom_height"
                                src="{{ get_post_image($media_file->file_name, 'optimized') }}" alt="">
                        </a>
                    </div>
                @endif
            @endforeach
        </div>
        <a href="{{ route('user.photos', [$user_data->id, 'identifire' => $viewData->profileIdentifier($page_identifire ?? null)]) }}"
    ...
    <div class="widget friend-widget">
        <div class="widget-header mb-3 d-flex justify-content-between align-items-center">
            ...
            <span>{{ $viewData->acceptedFriends($user_data)['count'] }} {{ get_phrase('Friends') }}</span>
        </div>
        <div class="row row-cols-3 g-1 mt-3">
            @foreach ($viewData->acceptedFriends($user_data)['items'] as $friend)
                <div class="col">
                    <a href="{{ route('user.profile.view', $friend->id) }}" class="friend d-block">
                        <img width="100%" class="h-90" src="{{ get_user_image($friend->photo, 'optimized') }}"
                            alt="">
                        <h6 class="small">{{ $friend->name }}</h6>
                    </a>
                </div>
            @endforeach
        </div>
