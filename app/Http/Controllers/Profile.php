...
class Profile extends Controller
{
    ...
    public function album($action_type, Request $request)
    {
        // return $action_type;
        $error = null;
        ...
        if ($action_type == 'form') {
            ...
            $response = ['hideCustomModal' => 1, 'appendAfterElement' => '#'.$album_show_on.'-album-row .col-create-album:first-child', 'content' => $album_view];

            echo json_encode($response);
        }
    }

    public function load_albums(Request $request)
    {
        $all_albums = Albums::where('user_id', $this->user->id)
            ->whereNull('page_id')
            ->whereNull('group_id')
    ...
    public function load_my_friend_requests(Request $request)
    {
        $friend_requests = Friendships::where('accepter', $this->user->id)
            ->where('is_accepted', '!=', 1)
            ->skip($request->offset)->take(15)->get();
    ...
    public function about($action_type, Request $request)
    {
        $response = [];
        ...
        if ($action_type = 'update') {
            ...
            $response = ['alertMessage' => get_phrase('Your bio updated'), 'hideElem' => '.edit-bio-form', 'showElem' => '.edit-bio-btn', 'elemSelector' => '.my-about', 'content' => htmlspecialchars(nl2br($request->about))];
        }

        return json_encode($response);
    }

    public function my_info($action_type, Request $request)
    {
        $response = [];
        ...
        if ($action_type == 'edit') {
            ...
            $response = ['hideCustomModal' => 1, 'elemSelector' => '#my-profile-info', 'content' => $user_frofile_info, 'alertMessage' => get_phrase('Profile info updated')];
        }
