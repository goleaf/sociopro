...
| Page IDOR / XSS | Page edit, cover, and info endpoints accepted authenticated non-owner access; the edit modal rendered page descriptions as raw HTML. | Added `PagePolicy`, Page Form Requests, controller `Gate::authorize()` checks, modal-level owner authorization, escaped textarea rendering, category existence validation, and regression tests in `tests/Feature/PageSecurityPerformanceTest.php`. |
...
| Queries inside Blade | Medium | Page create/edit modals and page cards were fixed in this pass; other legacy views still need review. | Move one remaining high-traffic partial at a time into controller/view-model data with query-count tests. |
