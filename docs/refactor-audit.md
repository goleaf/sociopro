...
| 8 | High | Stop exposing secrets and privileged gateway values to the browser. | payment settings views, `config/paypal.php` | Move runtime secrets to `.env` + `config()` or encrypted storage. Verify admin-only visibility for payment credentials. |
