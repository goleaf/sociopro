...
- Current app baseline from dependency files: PHP `^8.3`, Laravel `13.18.0`, PHPUnit `12.5.30`, Pint `1.29.3`, Vite / SCSS frontend build.
- `composer validate --strict`: valid.
- `composer audit --locked`: no security vulnerability advisories found.
- Earlier `npm audit --audit-level=moderate` failures were tied to the removed Laravel Mix / Webpack dependency chain. Re-run audit after dependency changes and record any new findings in `docs/known-technical-debt.md`.
...
| FE-005 | Legacy public theme/vendor assets still sit outside the Vite module pipeline. | Medium | High | Frontend updates can miss old global scripts/styles, and unsafe inline behavior can remain hidden in Blade or public assets. | Frontend/build | Retire public legacy assets route by route with visual smoke tests and rollback notes. | Inventory public assets used by one high-traffic page before moving behavior into Vite-managed modules. |

## Deployment Risks

| ID | Risk | Severity | Likelihood | Impact | Owner area | Recommended fix | Safe first step |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DEP-001 | CI must remain aligned with installed PHP/Node tooling. | Medium | Medium | Stale workflow versions or missing checks can let regressions through despite automation. | DevOps/QA | Keep `.github/workflows/ci.yml` in sync with Composer, Pint, PHPStan/Larastan, PHPUnit, frontend lint/style/format checks, and Vite build. | Run the same command set locally before changing CI. |
...
| ID | Risk | Severity | Likelihood | Impact | Owner area | Recommended fix | Safe first step |
| --- | --- | --- | --- | --- | --- | --- | --- |
| PKG-001 | Frontend dependency audit findings can drift after package updates. | Medium | Medium | Build tooling and browser dependencies can reintroduce security noise or unsupported packages. | Frontend/build | Keep `npm run audit:prod` and package-lock review in CI/release checks. | Re-run `npm audit --audit-level=moderate` after dependency updates and document any remaining actionable findings. |
