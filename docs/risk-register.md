...
| SEC-005 | Secrets and provider credentials can be rendered to Blade/JavaScript, including Hugging Face, Zoom, and payment-related values. | Critical | High | Browser exposure of API/provider secrets, payment keys, or meeting credentials. | Security/frontend/payments | Move provider calls server-side, use config-backed secrets, redact admin displays, and avoid browser-visible secret values. | Add a view test for one page proving the Hugging Face token is not rendered. |
...
| ID | Risk | Severity | Likelihood | Impact | Owner area | Recommended fix | Safe first step |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DEP-001 | No `.github/workflows` directory is present. | High | High | Tests, Pint, audits, and builds depend on local discipline rather than automated gates. | DevOps/QA | Add CI for Composer install, `php artisan test`, Pint, Composer validation/audit, npm audit/build when relevant. | Create a docs-only CI plan, then add a minimal test workflow in a separate commit. |
...
| DEP-005 | Rollback strategy for schema, assets, queues, and config is not formalized. | High | Medium | Bad deploys may be hard to reverse cleanly. | DevOps/database | Document rollback steps per release type. | Add a rollback section to deployment docs before the next schema-affecting change. |
...
| PKG-004 | Optional quality tools are not installed: Larastan/PHPStan, Rector, ESLint, Stylelint, Prettier. | Medium | High | Type, style, JS, and CSS issues depend on manual review. | Tooling/QA | Add tools incrementally with baselines and focused rules. | Add Larastan/PHPStan in a dedicated tooling commit after critical security fixes are isolated. |
...
| TEST-005 | External provider tests are uneven; PayPal/Paystack have coverage, but broader payment/AI/upload flows need fakes. | High | Medium | Real services may be called or broken behavior may ship untested. | QA/integrations | Use `Http::fake()`, storage fakes, queue fakes, and provider contract tests. | Add fake-backed tests for Hugging Face/AI image generation or move it backend-first. |
...
3. Stop browser exposure of provider secrets, beginning with Hugging Face/AI image generation.
