...
| `vendor/bin/pint --test` | Fails in the current dirty checkout on unrelated broader-refactor files: `app/ViewModels/BladeViewData.php`, `app/Http/Controllers/ApiController.php`, `app/Http/Controllers/AdminCrudController.php`, `app/Http/Controllers/MainController.php`, `tests/Feature/EloquentCastAuditTest.php`, and `tests/Feature/EloquentRelationshipAuditTest.php`. The page-hardening files were formatted with Pint and the focused tests pass. | The dirty checkout cannot claim a green full Pint gate until the broader refactor files are formatted. | Run `vendor/bin/pint` on the broader refactor before committing those unrelated files, then rerun `vendor/bin/pint --test`. |
